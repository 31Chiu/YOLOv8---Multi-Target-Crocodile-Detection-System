...
def process_base_directory(base_image_dir, base_label_dir, model):
    """
    Scans a base directory and processes each one, returning all found image paths.
    """
    all_image_paths = []
...
def main():
    ...
    # 4.1 Automatically generate a CVAT-specific train.txt manifest
    ...
    # 4.2 Generate the validation manifest (Note: To comply with CVAT's strict formatting requirements, the internal path must still include obj_train_data)
    if val_image_paths:
